...
async function warnIfOutdated(repo, clonedRepoDir, version) {
    if (repo === CHA_URL) {
        const rootPkgPath = join(clonedRepoDir, 'packages/create-hyperweb-app/package.json');
        const rootPkg = JSON.parse(fs.readFileSync(rootPkgPath, 'utf-8'));
        if (semver.lt(rootPkg.version, version)) {
            console.warn(c.yellow(`⚠️ You are using create-hyperweb-app version ${c.red(rootPkg.version)}, but version ${c.green(version)} is available. Run "${c.cyan('cca upgrade')}" or "${c.cyan('npm install -g create-hyperweb-app@latest')}" to upgrade.`));
        }
    }
}
